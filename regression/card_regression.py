...
class Diffusion(object):
    ...
    def evaluate_guidance_model_on_both_train_and_test_set(
        self, train_set_object, train_loader, test_set_object, test_loader
    ):
        y_train_rmse_aux_model = self.evaluate_guidance_model(
            train_set_object, train_loader
        )
        y_test_rmse_aux_model = self.evaluate_guidance_model(
            test_set_object, test_loader
        )
        logging.info(
            (
                "{} guidance model un-normalized y RMSE "
                + "\n\tof the training set and of the test set are "
                + "\n\t{:.8f} and {:.8f}, respectively."
            ).format(
                self.config.diffusion.conditioning_signal,
                y_train_rmse_aux_model,
                y_test_rmse_aux_model,
            )
        )
    ...
    def compute_unnorm_y(self, cur_y, testing):
        if testing:
            y_mean = (
                cur_y.cpu()
                .reshape(-1, self.config.testing.n_z_samples)
                .mean(1)
                .reshape(-1, 1)
            )
        else:
    ...
    def make_subplot_at_timestep_t(
        self, cur_t, cur_y, y_i, y_0, axs, ax_idx, prior=False, testing=True
    ):
        ...
        if prior:
            axs[ax_idx].set_title(r"$p({y}_\mathbf{prior})$", fontsize=23)
            axs[ax_idx].set_title(
                r"$p({y}_\mathbf{prior})$" + "\n" + kl_unnorm_str, fontsize=23
            )
            axs[ax_idx].legend()
        else:
            axs[ax_idx].set_title(r"$p(\mathbf{y}_{" + str(cur_t) + "})$", fontsize=23)
            axs[ax_idx].set_title(
                r"$p(\mathbf{y}_{" + str(cur_t) + "})$" + "\n" + kl_unnorm_str,
                fontsize=23,
            )
    ...
    def train(self):
        ...
        if (
            hasattr(config.diffusion.nonlinear_guidance, "apply_early_stopping")
        ...
        if (
            ...
            if (
                ...
                for epoch in range(
                    ...
                    y_val_rmse_aux_model = self.evaluate_guidance_model(
                    ...
                    if early_stopper.early_stop:
                        print(
                            (
                                "Obtained best performance on validation set after Epoch {}; "
                ...
                self.evaluate_guidance_model_on_both_train_and_test_set(
                    train_subset_object, train_subset_loader, val_set_object, val_loader
                )
                    ...
                    if step % self.config.training.snapshot_freq == 0 or step == 1:
                        ...
                        if step > 1:  # skip saving the initial ckpt
                            torch.save(
                                states,
                                os.path.join(
                                    self.args.log_path, "ckpt_{}.pth".format(step)
                                ),
                            )
                        ...
                        if (
                            ...
                            with torch.no_grad():
                                ...
                                if epoch == start_epoch:
                                    ...
                                    axs[0].set_title(
                                        r"$q(\mathbf{y}_{" + str(1) + "})$", fontsize=23
                                    )
                                    y_q_seq = []
                                    for j in range(1, self.num_figs):
                                        cur_t = j * self.vis_step
                                ...
                                for j in range(1, self.num_figs):
                                    cur_t = j * self.vis_step
                                    cur_y = (
                                        y_p_seq[self.num_timesteps - cur_t]
                                        .detach()
                                        .cpu()
                                    )
                                ...
                                axs[self.num_figs].set_title(
                                    r"$p({z}_\mathbf{prior})$", fontsize=23
                                )
                                # kl = kld(y_T, cur_y)
                                # kl_y0 = kld(y_batch.detach().cpu(), cur_y)
    ...
    def sample(self):
        ...
        if getattr(self.config.sampling, "ckpt_id", None) is None:
            states = torch.load(
                os.path.join(self.args.log_path, "ckpt.pth"), map_location=self.device
            )
            ckpt_id = "last"
        else:
            states = torch.load(
        ...
        if self.config.data.dataset == "swiss_roll":
            ...
            xy_0 = Gaussians().sample(
                self.config.sampling.sampling_size
                - self.config.sampling.sampling_size % 8,
                mode=8,
            )
        elif self.config.data.dataset == "25gaussians":
            xy_0 = Gaussians().sample(
                self.config.sampling.sampling_size
                - self.config.sampling.sampling_size % 25,
                mode=25,
            )
        ...
        y_batch = xy_0[:, -self.config.model.y_dim :]
        y_0_hat_batch = self.compute_guiding_prediction(
            x_batch.to(self.device), method=self.config.diffusion.conditioning_signal
        )
        y_T_mean = y_0_hat_batch
        ...
        with torch.no_grad():
            fig, axs = plt.subplots(
                1, self.num_figs, figsize=(self.num_figs * 8.5, 8), clear=True
            )
            for i in range(self.num_figs - 1):
                ...
                axs[i].set_title(
                    r"$q(\mathbf{y}_{" + str(i * self.vis_step) + "})$", fontsize=23
                )
            ...
            axs[self.num_figs - 1].set_title(
                r"$q(\mathbf{y}_{" + str(self.num_timesteps - 1) + "})$", fontsize=23
            )
            fig.savefig(
                os.path.join(
                    self.args.im_path,
                    "diffusion_samples_T{}_{}.png".format(self.num_timesteps, ckpt_id),
                )
            )
            ...
            fig, axs = plt.subplots(
                2, self.num_figs, figsize=(self.num_figs * 8.5, 8 * 2.2), clear=True
            )
            for i in range(self.num_figs - 1):
                cur_y = (
                    y_p_seq[self.num_timesteps - i * self.vis_step - 1].detach().cpu()
                )
                ...
                axs[1, i].set_title(
                    r"$p_{\theta}(\mathbf{y}_{" + str(i * self.vis_step) + "})$",
                    fontsize=23,
                )
                axs[1, i].set_xlabel(
                    "KL($q(y_t)||p(y_t)$)={:.2f}\nKL($q(y_0)||p(y_t)$)={:.2f}".format(
            ...
            axs[0, self.num_figs - 1].set_title(
                r"$q(\mathbf{y}_{" + str(self.num_timesteps) + "})$", fontsize=23
            )
            axs[1, self.num_figs - 1].scatter(x_batch, cur_y, s=10)
            axs[1, self.num_figs - 1].set_title(
                r"$p_{\theta}(\mathbf{y}_{" + str(self.num_timesteps) + "})$",
                fontsize=23,
            )
            ...
            fig.savefig(
                os.path.join(
                    self.args.im_path,
                    "generated_samples_T{}_{}.png".format(self.num_timesteps, ckpt_id),
                )
            )
            ...
            for i in range(self.num_figs - 1):
                ...
                kl = kld(y_q_seq[i].detach().cpu(), cur_y)
                kl_y0 = kld(y_q_seq[0].detach().cpu(), cur_y)
                heatmap_x, _, _ = np.histogram2d(
                    x_batch.numpy(), y_q_seq[i].numpy(), bins=100
                )
            ...
            kl_y0 = kld(y_q_seq[0].detach().cpu(), cur_y)
            heatmap_x, _, _ = np.histogram2d(
                x_batch.numpy(), y_q_seq[-1].numpy(), bins=100
            )
            ...
            fig.savefig(
                os.path.join(
                    self.args.im_path,
                    "generated_distribution_T{}_{}.png".format(
                        self.num_timesteps, ckpt_id
                    ),
                )
            )
        plt.close("all")
    ...
    def test(self):
        ...
        def compute_true_coverage_by_gen_QI(
            ...
            if verbose:
                y_true_below_0, y_true_above_100 = (
                    y_true_quantile_bin_count[0],
                    y_true_quantile_bin_count[-1],
                )
                logging.info(
                    (
                        "We have {} true y smaller than min of generated y, "
            ...
            y_true_ratio_by_bin = (
                y_true_quantile_bin_count_ / dataset_object.test_n_samples
            )
            assert (
        ...
        def store_gen_y_at_step_t(config, current_batch_size, idx, y_tile_seq):
            ...
            gen_y = (
                y_tile_seq[idx]
                .reshape(
                    current_batch_size, config.testing.n_z_samples, config.model.y_dim
                )
                .cpu()
                .numpy()
            )
        ...
        def set_NLL_global_precision(test_var=True):
            if test_var:
                ...
                if dataset_object.normalize_y:
                    y_test_unnorm = dataset_object.scaler_y.inverse_transform(
        ...
        def compute_batch_NLL2(config, dataset_object, y_batch, generated_y):
            """
            generated_y: has a shape of (current_batch_size, n_z_samples, dim_y)
        ...
        def handle_input(batch_y, cdf_at_borders, bin_masses, bin_borders):
            ...
            assert not (
                ...
                or (
                    torch.isnan(cdf_at_borders).any()
        ...
        if config.diffusion.conditioning_signal == "NN":
            aux_states = torch.load(
                os.path.join(log_path, "aux_ckpt.pth"),
                map_location=self.device,
                weights_only=True,
            )
            self.cond_pred_model.load_state_dict(aux_states[0], strict=True)
        ...
        x_check, y_check = (
            dataset_check[:, : -config.model.y_dim],
            dataset_check[:, -config.model.y_dim :],
        )
        y_check = y_check.to(self.device)
        y_0_hat_check = self.compute_guiding_prediction(
            x_check.to(self.device), method=config.diffusion.conditioning_signal
        )
        ...
        with torch.no_grad():
            ...
            for step, xy_batch in enumerate(test_loader):
                ...
                y_0_hat_tile = (
                    (
                        y_0_hat_batch.repeat(
                            config.testing.n_z_samples, 1, 1
                        ).transpose(0, 1)
                    )
                    .to(self.device)
                    .flatten(0, 1)
                ...
                if (
                    step % config.testing.plot_freq == 0
        ...
        if config.testing.compute_metric_all_steps:
            for idx in range(self.num_timesteps + 1):
                ...
                y_true_ratio_by_bin, qice_coverage_ratio, y_true = (
                    compute_true_coverage_by_gen_QI(
                        config=config,
                        dataset_object=dataset_object,
            ...
            n_metrics = 4
            fig, axs = plt.subplots(
                n_metrics, 1, figsize=(8.5, n_metrics * 3), clear=True
            )  # W x H
            ...
            # PICP
            picp_ideal = (
                config.testing.PICP_range[1] - config.testing.PICP_range[0]
            ) / 100
        ...
        if config.testing.make_plot:
            ...
            if config.data.no_multimodality:
                if not config.data.inverse_xy:
                    coverage, CI_y_pred, low, high = compute_PICP(
                        config=config,
                        y_true=y_true,
                        all_gen_y=all_gen_y,
                        return_CI=True,
            ...
            y_pred_mean = compute_prediction_SE(
                config=config,
                dataset_object=dataset_object,
            ...
            if dataset_object.normalize_y:
                all_true_y = dataset_object.scaler_y.inverse_transform(
                    all_true_y
                ).astype(np.float32)
                all_gen_y = all_gen_y.reshape(
                    dataset_object.test_n_samples * config.testing.n_z_samples,
                    config.model.y_dim,
                )
                all_gen_y = (
                    dataset_object.scaler_y.inverse_transform(all_gen_y)
                    .astype(np.float32)
                    .reshape(
            ...
            if config.data.inverse_xy:
                x_noiseless_mean = compute_y_noiseless_mean(
                    dataset_object,
                ...
                if config.model.y_dim == 1:
                    ...
                    ax1.plot(
                        x_noiseless_mean[sorted_idx_],
                        all_true_y[sorted_idx_],
                        c="orange",
                        alpha=1,
                ...
                if not config.data.inverse_xy:
                    ...
                    logging.info(
                        (
                            ...
                            np.sqrt(np.mean((y_pred_mean - y_noiseless_mean) ** 2))
                        )
                    )
            ...
            if config.testing.plot_true:
                ...
                true_sampled_idx = np.random.choice(
                    np.arange(n_total_true),
                    size=n_total_true // n_true_x_for_plot_scale,
                    replace=False,
                )
                ax1.scatter(
                    all_true_x[true_sampled_idx],
                    all_true_y[true_sampled_idx],
                    s=2,
                    c="r",
                    marker="o",
                    alpha=0.5,
            ...
            if config.testing.plot_gen:
                ...
                samp_idx = np.random.randint(
                    low=0,
                    high=config.testing.n_z_samples,
                    size=(
                        dataset_object.test_n_samples,
                ...
                if len(all_gen_y_.shape) == 3:
                    all_gen_y_ = all_gen_y_.reshape(
                        dataset_object.test_n_samples * n_samples_gen_y_for_plot,
                        config.model.y_dim,
                    )
                ...
                gen_sampled_idx = np.random.choice(
                    ...
                    // (n_true_x_for_plot_scale * n_samples_gen_y_for_plot),
                    replace=False,
                )
                ax1.scatter(
                    all_true_x_tile[gen_sampled_idx],
                    all_gen_y_[gen_sampled_idx],
                    s=2,
                    c="b",
                    marker="^",
                    alpha=0.5,
            ...
            fig_1.savefig(
                os.path.join(args.im_path, "gen_vs_true_scatter.png"),
                dpi=1200,
                bbox_inches="tight",
            )
            ...
            fig_2.savefig(
                os.path.join(args.im_path, "quantile_interval_coverage.png"), dpi=1200
            )
            ...
            ax3.set_title(
                "Ratio of True y vs. Optimal Ratio \nin Each Quantile Interval of Generated y"
            )
            ...
            fig_3.savefig(
                os.path.join(
                    args.im_path, "quantile_interval_coverage_true_vs_optimal.png"
                ),
                dpi=1200,
            )
            ...
            fig.tight_layout()
            fig.savefig(
                os.path.join(args.im_path, "gen_vs_true_distribution_vis.png"), dpi=1200
            )
